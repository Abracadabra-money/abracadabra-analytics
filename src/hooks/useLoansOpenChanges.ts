...
import { LOANS_OPEN_CHANGES } from "config/endpoints";
...
export interface LoansOpenChangesRes {
...
export interface LoansOpenChangesItem {
    readonly week: Date;
    readonly count: number;
...
export interface LoansOpenChanges {
    readonly loading: boolean;
    readonly error: boolean;
    readonly changes: LoansOpenChangesItem[];
}
...
export default function (group?: FeesChangesBody["group"]): LoansOpenChanges {
    ...
    const { data, error } = useAxios<LoansOpenChangesRes[]>(LOANS_OPEN_CHANGES, { method: "POST", data: body });

    return useMemo(() => {
        if (!data || error) {
            return { loading: !error && !data, error: !!error, changes: [] };
        }
        const changes = data.map(({ week, count, network, assettype, pool }) => {
